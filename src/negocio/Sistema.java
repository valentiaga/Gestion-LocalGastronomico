...
public class Sistema
{
	...
	// private HashMap<String, PromocionTemporal> promocionTemp =
	// Cerveceria.getInstance().getPromocionTemp();
...
//	}
	...
	/*
	 * public HashMap<String, PromocionTemporal> getPromocionTemp() { return
	 * promocionTemp; }
	 */
	...
	/*
	 * public FuncionalidadOperario login(String userName, String password) throws
	 * UserNameIncorrecto_Exception, ContrasenaIncorrecta_Exception,
	 * OperarioInactivo_Exception { FuncionalidadOperario funcionalidad = null;
	 * if(this.operariosRegistrados.containsKey(userName)) { Operario operario =
	 * this.operariosRegistrados.get(userName);
	 * if(operario.verificaPassword(password) == true) { //this.operarioActual =
	 * operario; // podriamos tener solo funcionalidad, porque tiene el operario
	 * if(operario.isActivo() == false) throw new
	 * OperarioInactivo_Exception("El usuario '"+userName+"' se encuentra inactivo."
	 * ); if(userName == this.usuarioAdministrador) // si la contrasena sigue siendo
	 * Admin1234 hay que obligarlo a cambiarla funcionalidad = new
	 * FuncionalidadAdmin(operario); else funcionalidad = new
	 * FuncionalidadOperario(operario); } else { throw new
	 * ContrasenaIncorrecta_Exception("Contrasena incorrecta."); } } else { throw
	 * new UserNameIncorrecto_Exception("El usuario '"
	 * +userName+"' no se encuentra registrado en el sistema."); } return
	 * funcionalidad; }
	 */
	 ...
	 * @param userName utilizado para logear al usuario. <br>
	 * @param password correspondiente al userName. <br>
	 * @throws UserNameIncorrecto_Exception   cuando el userName no se encuentra en
	 *                                        la coleccion de operariosRegistrados
	 * @throws ContrasenaIncorrecta_Exception cuando el password no corresponde al
	 *                                        password del operario.
			...
			throws UserNameIncorrecto_Exception, ContrasenaIncorrecta_Exception, OperarioInactivo_Exception
	{
		...
		if (this.operariosRegistrados.containsKey(userName))
		{
			...
			if (operario.verificaPassword(password) == true)
			{
				...
				if (userName.equals(this.usuarioAdministrador)) // si la contrasena sigue siendo Admin1234 hay que
																// obligarlo a cambiarla
		...
		{
			throw new UserNameIncorrecto_Exception(
					"El usuario '" + userName + "' no se encuentra registrado en el sistema.");
		}
	}
